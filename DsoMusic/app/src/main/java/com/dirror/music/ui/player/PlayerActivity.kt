...
import com.dirror.music.util.FloatingWindowPermissionUtil
...
//            }
    ...
    override fun initShowDialogListener() {
        binding.apply {
            ...
            // 悬浮歌词 - 点击开启/关闭，长按进入设置
            ivFloatingLyrics?.setOnClickListener {
                singleClick {
                    FloatingWindowPermissionUtil.checkAndRequestPermission(
                        this@PlayerActivity,
                        onGranted = {
                            // 开启悬浮歌词服务
                            FloatingLyricsService.startService(this@PlayerActivity)
                            toast("悬浮歌词已开启")
                            // 更新当前歌词
                            updateFloatingLyrics()
                        },
                        onDenied = {
                            toast("需要悬浮窗权限才能使用歌词悬浮功能")
                        }
                    )
                }
            }
            ivFloatingLyrics?.setOnLongClickListener {
                // 长按进入设置
                val intent = Intent(this@PlayerActivity, FloatingLyricsSettingsActivity::class.java)
    ...
    private fun updateFloatingLyrics() {
        // 从 ViewModel 获取当前歌词
        val lyricViewData = playViewModel.lyricViewData.value
        val lyrics = lyricViewData?.lyric
        if (!lyrics.isNullOrEmpty()) {
            val currentTime = App.musicController.value?.getProgress() ?: 0
            val (currentLyric, nextLyric) = findCurrentAndNextLyric(lyrics, currentTime)
            FloatingLyricsService.updateLyrics(this, currentLyric, nextLyric)
        }
    }

    /**
     * 根据时间查找当前歌词和下一句歌词
     * @return Pair<当前歌词, 下一句歌词>
     */
    private fun findCurrentAndNextLyric(lyrics: String, currentTime: Int): Pair<String, String> {
        val lines = lyrics.lines()
        val lyricLines = mutableListOf<Pair<Long, String>>() // <时间戳(毫秒), 歌词内容>
        
        // 解析所有歌词行 - 支持多种时间格式 [mm:ss.xx] 或 [mm:ss:xxx]
        for (line in lines) {
            if (line.contains("[")) {
                // 匹配时间戳 [mm:ss.xx] 或 [mm:ss:xxx]
                val timeMatches = Regex("""\[(\d{2}):(\d{2})[:\.](\d{2,3})\]""").findAll(line)
                val content = line.replace(Regex("""\[\d{2}:\d{2}[:\.]\d{2,3}\]"""), "").trim()
                ...
                if (content.isNotEmpty()) {
                    for (match in timeMatches) {
                        ...
                        // 处理两位或三位毫秒
                        val timeMs = (min * 60 + sec) * 1000L + if (ms < 100) ms * 10 else ms
                        lyricLines.add(Pair(timeMs, content))
                    }
                }
            }
        }
        ...
        var currentLyric = ""
        var nextLyric = ""
        val currentTimeMs = currentTime.toLong()
        
        for (i in lyricLines.indices) {
            if (lyricLines[i].first <= currentTimeMs) {
                currentLyric = lyricLines[i].second
                // 下一句
                if (i + 1 < lyricLines.size) {
                    nextLyric = lyricLines[i + 1].second
                }
            } else {
                break
            }
        ...
        return Pair(
            currentLyric.ifEmpty { customText },
            nextLyric
        )
    }
