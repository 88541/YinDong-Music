...
class PlayerViewModel: ViewModel() {
    ...
    fun updateLyric() {
        // 更改歌词
        App.musicController.value?.getPlayingSongData()?.value?.let { song ->
            com.dirror.music.service.ServiceSongUrl.getLyric(song) { lyricViewData ->
                runOnMainThread {
                    _lyricViewData.value = lyricViewData
                    if (lyricTranslation.value == true) {
                        this.lyricViewData.value = _lyricViewData.value
                    } else {
                        this.lyricViewData.value = LyricViewData(_lyricViewData.value!!.lyric, "true")
                    }
                }
            }
        }
