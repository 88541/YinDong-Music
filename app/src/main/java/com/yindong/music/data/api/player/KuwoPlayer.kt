...
/**
 * 酷我音乐播放API（TuneHub V3方式）
 */
object KuwoPlayer {

    private const val TAG = "KuwoPlayer"
     ...
     * 获取播放链接
     * @param songId 歌曲ID
     * @param type 音质类型: 128k, 320k, flac, flac24bit
     * @return 播放链接结果
     */
    suspend fun getMusicUrl(songId: String, type: String): MusicUrlResult {
        return withContext(Dispatchers.IO) {
            try {
                Log.d(TAG, "获取酷我播放链接: songId=$songId, type=$type")
                ...
                val quality = when (type.lowercase()) {
                    ...
                    "320k" -> "320k"
                    "flac" -> "flac"
                    "flac24bit", "hires", "master" -> "flac24bit"
                    else -> "320k"
                }
